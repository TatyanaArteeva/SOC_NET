...
const Group = (
    ...
    useEffect(() => {
        ...
        const information = async () => {
            try {
                ...
                const path = `/api/group-relation/get-group-accounts/${idInUrl}?start=0&end=6`;
                const userGroup = await Service.getUserGroup(path);
                if (!cleanupFunction) {
                    groupId(idInUrl)
                    localStorage.setItem('idGroup', idInUrl);
                    if (res.data.group.theme === "Другое") {
                        setThemeGroup(res.data.group.ownTheme)
                    } else if (res.data.group.theme === "Не выбрано") {
                        setThemeGroup('')
                    } else {
    ...
    function requestJoinAndExitGroup(path) {
    ...
    function addGroups() {
        requestJoinAndExitGroup(`/api/group-relation/join-group/${idInUrl}`)
    }

    function exitFromGroup() {
        requestJoinAndExitGroup(`/api/group-relation/leave-group/${idInUrl}`)
    }
    ...
    if (accesses.canModify) {
        btnModificationGroup = <div className="group__information__data__modification__wrapper">
            <button onClick={goToModification}
                className="group__information__data__modification__btn">
                Редактировать
            </button>
        </div>
    }
    ...
    if (themeGroup !== undefined && themeGroup.length > 0) {
        theme = <div className="group__information__data__content">
            <div className="group__information__data__content_title">
                Тема:
            </div>
            <span className="group__information__data__content_content">
                {themeGroup}
            </span>
        </div>
    }
    ...
    if (subThemeGroup !== undefined && subThemeGroup.length > 0) {
        subTheme = <div className="group__information__data__content">
            <div className="group__information__data__content_title">
                Подтема:
            </div>
            <span className="group__information__data__content_content">
    ...
    if (descriptionGroup !== undefined && descriptionGroup.length > 0) {
        description = <div className="group__information__data__content">
            <div className="group__information__data__content_title">
                Описание:
            </div>
            <span className="group__information__data__content_content">
    ...
    if (errorMessage.length > 0) {
        modalWindowErrorActionsUserGroup = <div className="group__overlay"
            onClick={closeModalWindowErrorMessage}>
            <div className="group__modal"
                onMouseLeave={inBlockErrorMessageFalse}
                onMouseEnter={inBlockErrorMessageTrue}>
                <div className="group__modal_cancel">
                    <img src={cancel}
                        alt="cancel"
                        onClick={closeModalErrorForActionsUser}
                    />
                </div>
            ...
            <div className="group__photo-and-btns__wrapper">
                <div className="group__photo-and-btns__photo">
                    <div className="group__photo-and-btns__photo__wrapper">
                        <img className="group__photo-and-btns__photo__photo-group"
                            src={photoGroup}
                            alt="photoGroup"
                        />
                    </div>
                ...
                <div className="group__photo-and-btns__btns">
                    <div className="group__photo-and-btns__btns__navigation">
                        <button className="group__photo-and-btns__btns__navigation__main-btn">
                            Навигация
                        </button>
                        <div className="group__photo-and-btns__btns__navigation__menu">
                            <div className="group__photo-and-btns__btns__navigation__wrapper">
                                <Link to={idUserForUrl}>
                                    <img className="group__photo-and-btns__btns__navigation__item"
                                        src={home}
                                        alt="Домой"
                                    />
                                </Link>
                                <Link to={idUserForUrl}>
                                    <span className="group__photo-and-btns__btns__navigation__item__label">
                                        Домой
                                    </span>
                                </Link>
                            </div>
        ...
        <div className="group__information">
            <div className="group__information__data">
                ...
                <div className="group__information__data__content__wrapper">
                    <div className="group__information__data__content">
                        <span className="group__information__data__content_name">
                            {nameGroup}
                        </span>
                    </div>
                    {theme}
                    ...
                    <div className="group__information__data__content">
                        ...
                        <span onClick={goToPageAdminGroup}
                            className="group__information__data__content_content">
                            {administratorGroup.firstName} {administratorGroup.lastName}
                        </span>
                    </div>
                </div>
            </div>
            <div className="group__information__participants">
                <div className="group__information__participants__count-and-btn">
                    <div className="group__information__participants__count-and-btn_count">
                        <div>
                            Участники группы:
                        </div>
                        <span>
                            {totalSizeUserInGroup}
                        </span>
                    </div>
                    <button className="group__information__participants__count-and-btn_btn"
                        onClick={openAllUserGroup}>
                        Показать всех
                    </button>
