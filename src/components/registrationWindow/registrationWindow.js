...
class RegistrationWindow extends Component {
    constructor(props) {
        ...
        const { Service, registrationSuccessful, closeModalRegistration } = this.props;
    ...
    render() {
        ...
        const { birthDay, closeModalRegistration } = this.props;
        ...
        if (errorMessage.length > 0) {
            errorModal = <div className="passwordRequirements">{errorMessage}</div>
        }
        ...
        const contentRegistration = <form onSubmit={this.postFormRegistration}
            className="registration-window__modal">
            <h2 className="registration-window__modal__title">Регистрация</h2>
            <input onChange={this.valueFirstName}
                ...
                value={firstName}
            />
            <input onChange={this.valueLastName}
                ...
                value={lastName}
            />
            <input onChange={this.valueEmail}
            ...
            {errorModal}
            <button onClick={this.valueBirtDay}
                type="submit"
                className="registration-window__modal__btn">
                Зарегистрироваться
            </button>
            <button onClick={() => closeModalRegistration()}
                className="registration-window__modal__btn">
                Вернуться
            </button>
        </form>

        const content = spinner ? <Spinner /> : contentRegistration
